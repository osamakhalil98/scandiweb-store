    ...
    if (this.clickedItems.length === 6) {
      if (name === "Capacity") {
        this.clickedItems.forEach((item, index) => {
          ...
          if (item[idx].getAttribute("data-item") === `${idx}`) {
            console.log("hi");
            if (item[idx].classList.contains(styles.notSelectedAttr)) {
              item[idx].classList.remove(styles.notSelectedAttr);
              item[idx].classList.add(styles.selectedAttr);
            } else {
              item[idx].classList.toggle(styles.selectedAttr);
            }
          } else {
            item[idx].classList.add(styles.notSelectedAttr);
          }
        });
      } else if (nam === "With USB 3 ports") {
        this.clickedItems.forEach((item, index) => {
