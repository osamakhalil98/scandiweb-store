...
export default class Card extends Component {
  ...
  clickedItems = [];
  handleAttrSelect = (type, idx) => {
    if (this.clickedItems.length === 6) {
    } else {
      this.clickedItems.forEach((item, index) => {
        console.log(item[index].getAttribute("data-item"));
        if (item[index].getAttribute("data-item") === `${idx}`) {
          console.log("hi");
          if (item[index].classList.contains(styles.notSelectedAttr)) {
            item[index].classList.remove(styles.notSelectedAttr);
            item[index].classList.add(styles.selectedAttr);
          } else {
            item[index].classList.toggle(styles.selectedAttr);
          }
        } else {
          item[index].classList.add(styles.notSelectedAttr);
        }
      });
    }
  };
