...
export default class Card extends Component {
  ...
  handleAttrSelect = (name, idx) => {
    if (this.clickedItems.length === 6) {
      if (name === "Capacity") {
        for (let i = 0; i < 2; i++) {
          if (this.clickedItems[idx].getAttribute("data-item") === `${idx}`) {
            console.log("hi");
            if (
              this.clickedItems[idx].classList.contains(styles.notSelectedAttr)
            ) {
              this.clickedItems[idx].classList.remove(styles.notSelectedAttr);
              this.clickedItems[idx].classList.add(styles.selectedAttr);
            } else {
              this.clickedItems[idx].classList.toggle(styles.selectedAttr);
            }
          } else {
            this.clickedItems[idx].classList.add(styles.notSelectedAttr);
          }
        }
