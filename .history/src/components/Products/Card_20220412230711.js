...
export default class Card extends Component {
  ...
  handleAttrSelect = (name, idx) => {
    if (this.clickedItems.length === 6) {
      if (name === "Capacity") {
        for (let i = 0; i < 2; i++) {
          console.log(this.clickedItems[i][idx]);
          if (
            this.clickedItems[i][idx].getAttribute("data-items") === `${idx}`
          ) {
            console.log("hi");
            if (
              this.clickedItems[i][idx].classList.contains(
                styles.notSelectedAttr
              )
            ) {
              this.clickedItems[i][idx].classList.remove(
                styles.notSelectedAttr
              );
              this.clickedItems[i][idx].classList.add(styles.selectedAttr);
            } else {
              this.clickedItems[i][idx].classList.toggle(styles.selectedAttr);
            }
          } else {
            this.clickedItems[i][idx].classList.add(styles.notSelectedAttr);
          }
        }
