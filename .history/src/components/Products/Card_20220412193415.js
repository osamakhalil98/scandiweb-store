...
export default class Card extends Component {
  ...
  handleAttrSelect = (type, idx) => {
    let attrArray = this.state.itemAttributes;
    if (!attrArray.includes(type)) {
      this.clickedItems[idx].classList.toggle(styles.selectedAttr);
      attrArray.push(type);
    } else {
      this.clickedItems[idx].classList.toggle(styles.selectedAttr);
      attrArray.filter((att) => att !== type);
    }

    this.setState({
      itemAttributes: [...attrArray],
    });
  };
  render() {
    return (
                  ...
                  <div style={{ display: "flex", marginLeft: "20px" }}>
                    {product?.items?.map((value, idx) => (
                      <p
                        ...
                        onClick={() => this.handleAttrSelect(product.type, idx)}
                        data-item-idx
                        ref={(ref) => (this.clickedItems[idx] = ref)}
                      >
                        <p
                          style={{
                            textAlign: "center",
